...
proc internalMeasure(text: string, fontSize: float, font: string, baseline: string): Vec2[float] =
...
proc createText*(props: TextProps = TextProps(), elemProps: ElemProps = ElemProps()): Element =
  newElement(
    ...
    layout = some(Layout(
      ...
      measure: (self: Element, avSize: Vec2[float]) => internalMeasure(props.text, props.fontSize.get(12.0), props.font.get("system-ui"), "top")
    )),
